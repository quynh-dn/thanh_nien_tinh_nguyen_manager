...
class LichPhongVanThamGiaSceen extends StatelessWidget {
  const LichPhongVanThamGiaSceen({super.key});
  @override
...
class _BodyState extends State<Body> {
  ...
  Future<List<LichPhongVan>> callApiData() async {
    ...
    try {
      ...
      var findId = "thanhPhanThamDu~'*[$id,*' or thanhPhanThamDu~'*, $id,*' or thanhPhanThamDu~'*, $id]*' or thanhPhanThamDu~'*[$id]*' ";
      var response;
      if (search != "") {
        response = await httpGet("/api/lich-phong-van/get/page?filter=$findId and $search&sort=status&sort=id,desc&page=${curentPage - 1}&size=$selectedValueRpp", context);
      } else {
        response = await httpGet("/api/lich-phong-van/get/page?filter=$findId&sort=status&sort=id,desc&page=${curentPage - 1}&size=$selectedValueRpp", context);
      }
  ...
  void btnSearch() async {
    setState(() {
      ...
      if (tuNgay != null) {
        findTuNgay = "and thoiGian>:'${DateFormat('dd-MM-yyyy').format(tuNgay!)} 00:00:00' ";
      } else {
        findTuNgay = "";
      }
      if (denNgay != null) {
        findDenNgay = "and thoiGian<:'${DateFormat('dd-MM-yyyy').format(denNgay!)} 23:59:59' ";
      } else {
        findDenNgay = "";
      }
        ...
        ? ListView(
            ...
            children: [
              const BreadCrumb(
                ...
                content: "Lịch phỏng vấn tham gia",
              ),
              Container(
                padding: paddingPage,
                child: Column(
                    ...
                    for (int i = 0; i < listData.length; i++)
                      DataRow(
                        cells: <DataCell>[
                          ...
                          DataCell(SizedBox(width: 300, child: SelectableText(listData[i].title ?? "", style: textDataRow))),
                          DataCell(SelectableText((listData[i].thoiGian != null) ? DateFormat('HH:mm dd-MM-yyyy').format(DateTime.parse(listData[i].thoiGian!).toLocal()) : "", style: textDataRow)),
                          DataCell(SizedBox(width: 300, child: SelectableText((listData[i].diaDiem != null) ? listData[i].diaDiem! : "", style: textDataRow))),
