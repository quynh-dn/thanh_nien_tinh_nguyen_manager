...
class PTSKSceen extends StatelessWidget {
  const PTSKSceen({super.key});
...
class _BodyState extends State<Body> {
    ...
    try {
      ...
      if (search != "") {
        response = await httpGet(
            "/api/phong-trao-su-kien/get/page?filter=$search&sort=status&page=${curentPage - 1}&size=$selectedValueRpp",
            context);
      } else {
        response = await httpGet(
            "/api/phong-trao-su-kien/get/page?page=${curentPage - 1}&sort=status&size=$selectedValueRpp",
            context);
      }
      ...
      if (response.containsKey("body")) {
        ...
        if (body["success"] == true) {
          ...
          for (var element in listPTSK) {
            if (element.status == 1 &&
                DateTime.now()
                        .difference(DateTime.parse(element.endDate!))
                        .inHours >
                    0) {
              setState(() {
                element.status = 2;
            ...
            var responseSLTG = await httpGet(
                "/api/phong-trao-su-kien/get/soluongtnv/${element.id}",
                context);
  ...
  changeStatus(PhongTraoSuKien data) async {
    print("oke");
    await httpPut(
        "/api/phong-trao-su-kien/put/${data.id}", data.toJson(), context);
  }
        ...
        ? ListView(
            ...
            children: [
              const BreadCrumb(
                ...
                content: "Quản lý phong trào sự kiện",
              ),
              Container(
                padding: paddingPage,
                child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Expanded(
                                    child: Text(
                                        "Từ: ${DateFormat('dd-MM-yyyy').format(DateTime.parse(elementPTSK.startDate!).toLocal())}",
                                        style: textDropdownTitle)),
                                const SizedBox(
                                  width: 10,
                                ),
                                Expanded(
                                    child: Text(
                                        "Đến: ${DateFormat('dd-MM-yyyy').format(DateTime.parse(elementPTSK.endDate!).toLocal())}",
                                        style: textDropdownTitle))
                              ],
                            ),
                            const SizedBox(height: 15),
                            Text(
                                "Số lượng tham gia: ${elementPTSK.soLuongDaDangKy}/${elementPTSK.soLuongHoTro}",
                                style: textDropdownTitle),
                          ],
                        ),
                      ),
