...
class CapNhatTNVThamGia extends StatefulWidget {
  TnvPtsk data;
  Function? callBack;
  CapNhatTNVThamGia({Key? key, this.callBack, required this.data}) : super(key: key);
  @override
  State<CapNhatTNVThamGia> createState() => _AddCategoryState();
}

class _AddCategoryState extends State<CapNhatTNVThamGia> {
  ...
  void initState() {
    super.initState();
    data = widget.data;
    selectedTNV = data.nguoiDung!;
    selectedPTSK = data.phongTraoSuKien!;
    selectStatus = Status(
        id: data.status,
        name: (data.status == 0)
            ? "Đăng ký"
                ...
                ElevatedButton(
                  onPressed: () async {
                    if (selectedPTSK.id == null || selectedTNV.id == null || selectStatus.id == null) {
                      ...
                      await httpPut("/api/tnv-ptsk/put/${data.id}", data.toJson(), context);
                      showToast(
                        context: context,
