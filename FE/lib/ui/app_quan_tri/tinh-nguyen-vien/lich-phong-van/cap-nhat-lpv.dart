...
class CapNhatLPV extends StatefulWidget {
  LichPhongVan data;
  Function? callBack;
  CapNhatLPV({
    super.key,
  ...
  @override
  State<CapNhatLPV> createState() => _BodyState();
}

class _BodyState extends State<CapNhatLPV> {
  //Load
  ...
  List<SinhVienDangKy> listSinhVien = [
    SinhVienDangKy(id: 0, fullName: "--Chọn sinh viên-", maSV: "")
  ];
  List<int> listCheckSV = [];

  List<TinhNguyenVien> listTNV = [
    TinhNguyenVien(id: 0, fullName: "--Chọn sinh viên-", maSV: "")
  ];
  List<int> listCheckTNV = [];
  ...
  Future<List<SinhVienDangKy>> callSinhVienDangKy(List<int> khac) async {
    ...
    try {
      var response = await httpGet(
          "/api/sinh-vien-dang-ky/get/page?filter=status:0 $searchKhac",
          context);
  ...
  void initState() {
    ...
    chonNgay = DateTime.parse(data.thoiGian!).toLocal();
    chonGio =
        DateFormat('HH:mm').format(DateTime.parse(data.thoiGian!).toLocal());
    content.text = data.content ?? "";
    diaDiem.text = data.diaDiem ?? "";
    listSinhVien = data.listSinhVienPv;
    listTNV = data.listThanhPhanThamDu;
    listCheckSV = data.sinhVienPvConvert ?? [];
    listCheckTNV = data.thanhPhanThamDuConvert ?? [];
    if (data.status == 0) {
      selectStatus = Status(id: 0, name: "Chưa phỏng vấn");
    } else if (data.status == 1) {
      selectStatus = Status(id: 1, name: "Đã phỏng vấn");
    } else if (data.status == 2) {
      selectStatus = Status(id: 2, name: "Huỷ");
    }
                          ...
                          children: [
                            DropdownSearch<SinhVienDangKy>(
                              asyncItems: (String? filter) =>
                                  callSinhVienDangKy(listCheckSV),
                              itemAsString: (SinhVienDangKy? u) =>
                                  "${u!.fullName} - ${u.maSV}",
                                    ...
                                    children: [
                                      SizedBox(
                                        ...
                                        child: IconButton(
                                            onPressed: () async {
                                              await httpGet(
                                                  '/api/sinh-vien-dang-ky/reset/${listCheckSV[i]}',
                                                  context);
                                              setState(() {
                                      ...
                                      SizedBox(
                                        ...
                                        child: IconButton(
                                            onPressed: () {
                                              setState(() {
                                                listSinhVien.add(SinhVienDangKy(
                                                    id: 0,
                                                    fullName:
                                                        "--Chọn sinh viên-",
                                                    maSV: ""));
                        ...
                        child: Row(
                          ...
                          children: [
                            DropdownSearch<TinhNguyenVien>(
                              asyncItems: (String? filter) =>
                                  callTNV(listCheckTNV),
                              itemAsString: (TinhNguyenVien? u) =>
                                  "${u!.fullName} - ${u.maSV}",
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  if (title.text == "" ||
                    ...
                    data.thoiGian =
                        "${DateFormat('yyyy-MM-dd').format(chonNgay!)}T$chonGio:00";
                    ...
                    if (data.status == 2) {
                      for (var element in listCheckSV) {
                        httpGet("/api/sinh-vien-dang-ky/reset/$element", context);
                      }
                    }

                    await httpPut("/api/lich-phong-van/put/${data.id}",
                        data.toJson(), context);
                    showToast(
                      context: context,
  ...
  renderStatus() {
    return DropdowSearchComon(
      ...
      widthDropdow: 300,
      marginDropdow: EdgeInsets.only(right: 60, bottom: 25),
    );
  }
}
