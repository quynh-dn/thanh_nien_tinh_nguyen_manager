...
import 'sua-poster.dart';
import 'them-poster.dart';

class QuanLyPosterSceen extends StatelessWidget {
  const QuanLyPosterSceen({super.key});
  @override
...
class _BodyState extends State<Body> {
  ...
  Future<List<Poster>> callApiData() async {
    ...
    try {
      var response;
      if (search != "") {
        response = await httpGet("/api/poster/get/page?filter=deleted:false and $search&page=${curentPage - 1}&size=$selectedValueRpp", context);
      } else {
        response = await httpGet("/api/poster/get/page?filter=deleted:false&page=${curentPage - 1}&size=$selectedValueRpp", context);
      }
          ...
          children: [
            BtnAppWidget(
              onPressed: () async {
                ...
                await showDialog(
                    context: context,
                    builder: (BuildContext context) => ThemMoiPoster(
                          callBack: (value) {
                            statusChange = value;
                          },
                        ));
                    ...
                    for (int i = 0; i < listData.length; i++)
                      DataRow(
                        cells: <DataCell>[
                          ...
                          DataCell(TextButton(onPressed: () {}, child: Text(listData[i].name ?? "", style: textDataRow))),
                          DataCell(SelectableText(listData[i].stt.toString(), style: textDataRow)),
                                  ...
                                  child: Container(
                                      ...
                                      child: InkWell(
                                          onTap: () {
                                            showDialog(
                                                context: context,
                                                builder: (BuildContext context) => SuaPoster(
                                                      data: listData[i],
                                                      callBack: (value) {
                                                        setState(() {
                                                          listData[i] = value;
                                                      ...
                                                      content: Container(
                                                        ...
                                                        child: Column(
                                                          ...
                                                          children: [
                                                              ...
                                                              children: [
                                                                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                                                  SizedBox(
                                                                    child: Row(
                                                                      children: [
                                                                        Text('Xác nhận xoá poster', style: textTitleAlertDialog),
                                                                      ],
                                                                    ),
                                                                  ),
                                                                  IconButton(
                                                            ...
                                                            Flexible(
                                                              child: Text("Xoá poster: ${listData[i].name}", style: textNormal),
                                                            ),
                                                            Column(
                                                              children: [
                                                                const SizedBox(height: 20),
                                                                ...
                                                                Row(
                                                                  ...
                                                                  children: [
                                                                    ElevatedButton(
                                                                      onPressed: () async {
                                                                        var response = await httpDelete("/api/poster/del/${listData[i].id}", context);

                                                                        showToast(
                                                                          context: context,
                                                                          msg: "Xoá poster thành công",
